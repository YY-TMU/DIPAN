        ...
        if feature == "intron" and int(rank) == intronrank:
            ...
            if len(exon_iv) == 1:
                ...
                if sum(np.array(exon_abundance)>10) == len(all_bamfiles) and sum(np.array([x-y for x,y in zip(exon_abundance,IPA_isoform_abundance)])>0)>len(all_bamfiles)*0.5:
                    ...
                    result = [SYMBOL,intron_rank,IPA_inf,IPAtype]+IPARatio_list
    return result
...
def output_IPUI(outfile,all_bamfiles,result_list):
    ...
    first_line = first_line + filenames
...
def output_exoncount(all_bamfiles,save_dir,result_list):
    save_dir = os.path.join(save_dir,"project/")
    if os.path.exists(save_dir) == False:
        os.makedirs(save_dir)
    for i in range(len(all_bamfiles)):
        bamfile_name = all_bamfiles[i]
        out = open(save_dir + bamfile_name.split("/")[-1].split(".")[0] + "_exoncount.txt","w")
        for gene_list in result_list:
...
output_exoncount(all_bamfiles,os.path.dirname(args.outfile),result_list)
