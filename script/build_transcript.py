...
def handleComposite(rt_pa_composite,exon_df):
    ...
    for x in rt_pa_composite.itertuples():
        gene_name = x.SYMBOL
        predic_te_region = x.Terminal_exon
        chrom,start,end = re.search("(chr.*?):(\d+)-(\d+)",predic_te_region).groups()
        ...
        if strand == "+":
            ...
            if not template_exon_df.empty:
                ...
                composite_infos.append([te_region,up_site,gene_id,template_transcript,predic_te_region,group])
            else:
                raise Exception("Check " + predic_te_region)
        elif strand == "-":
...
def handleSkip(rt_pa_skip,exon_df):
    ...
    for x in rt_pa_skip.itertuples():
        ...
        if strand == "+":
            ...
            up_exon_coordinates = list(find_up_exon_df["start"])
            up_site = ":".join([chrom,str(min(up_exon_coordinates)+1),strand])
        skip_infos.append([te_region,up_site,gene_id,template_transcript,predic_te_region,group])
    return skip_infos


def extractUpInfo(finder_pa_file,gtf_path):
    rt_pa = pd.read_table(finder_pa_file)
    all_genes = rt_pa["SYMBOL"].to_list()
    ...
    total_info_df = pd.DataFrame(total_infos,
                                 columns = ["region1","up_site","gene_id","transcript_id","region2","group"])
    return total_info_df


def splitTerminal(terminal_exon_df):
    novel_terminal_genes = terminal_exon_df["gene_id"].to_list()
    # split te
...
def transGTFormat(region,
    ...
    if annotation == "protein_coding":
        ...
        novel_transcript_id = "|".join(["novel_XM",template_transcript,te_type,"_".join(region.split(":")[1:3])])    
    elif annotation == "non_coding":
        novel_transcript_id = "|".join(["novel_XR",template_transcript,te_type,"_".join(region.split(":")[1:3])])
    exon_id = 'exon_id "' + novel_transcript_id + '.%d";'
    novel_transcript_id = 'transcript_id "' + novel_transcript_id + '";'
    # transcript info
    transcript_attr = "".join([gene_id,novel_transcript_id,gene_name])
