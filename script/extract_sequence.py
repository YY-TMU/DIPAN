...
def ivFromGtf(gtf_file_path):
    ...
    for gtf_line in gtf_lines:
        ...
        if not re.search("[M|_]",chrom):
            ...
            if transcript_id.startswith("novel"):
                novel_transcripts.add(transcript_id)
    # dataframe
    ...
    return iv_df,novel_transcripts


def skipTe(skip_transcripts,iv_df):
    skip_te_aa_dic = dict()
...
def compositeTe(composite_transcripts,iv_df):
...
def extractSeq(cds_fa,te_aa_dic):
    ...
    name_trans = {x.replace(">","").split(" ")[0]:x for x in name_trans if x.startswith(">novel")}            
    transcript_aa_seq_dic =  {k.replace(">","").split(" ")[0]:"".join(v) 
                              for k,v in transcript_aa_seq_dic.items()
                              if k.startswith(">novel")}
    # sequences for kmer
...
skip_te_aa_dic = skipTe(skip_transcripts,iv_df)
composite_transcripts = [x for x in novel_transcripts if ("Composite" in x) and ("novel_XM" in x)]
composite_te_aa_dic = compositeTe(composite_transcripts,iv_df)
